...
TARGET_TAG=v0.0.30 \
/tmp/upstream-ops-upgrade-kit-v0.0.30/scripts/upgrade.sh
```
...
TARGET_TAG=v0.0.30 /tmp/upstream-ops-upgrade-kit-v0.0.30/scripts/upgrade.sh
```
...
```bash
chmod +x /tmp/upstream-ops-upgrade-kit-v0.0.30/scripts/upgrade-to-postgres.sh
ENV_FILE=/srv/upstream-ops/.env \
DATA_DIR=/srv/upstream-ops/data \
COMPOSE_FILE=/srv/upstream-ops/docker-compose.yml \
POSTGRES_COMPOSE_FILE=/tmp/upstream-ops-upgrade-kit-v0.0.30/docker-compose.postgres.yml \
TARGET_TAG=v0.0.30 \
MIGRATION_IMAGE_TAG=v0.0.30 \
/tmp/upstream-ops-upgrade-kit-v0.0.30/scripts/upgrade-to-postgres.sh
```
...
.\scripts\upgrade-to-postgres.ps1 `
  -ComposeFile 'D:\upstream-ops\docker-compose.yml' `
  -PostgresComposeFile 'C:\Temp\upstream-ops-upgrade-kit-v0.0.30\docker-compose.postgres.yml' `
  -EnvFile 'D:\upstream-ops\.env' `
  -DataDir 'D:\upstream-ops\data' `
  -TargetTag 'v0.0.30' `
  -MigrationImageTag 'v0.0.30'
```
